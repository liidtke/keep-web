...
let toList (collection: IQueryable<'T>) = collection.ToList()
...
module Student =
    ...
    let sortMe (order: string) (query: IQueryable<Student>) =
        if order = "latest" then
            query.OrderByDescending(fun x -> x.CreationDate)
        else
            query.OrderBy(fun x -> x.Name)
    ...
    let filter (ctx: IMongoContext) (filters: StudentFilters) =
        if String.IsNullOrEmpty(filters.Filter) && filters.LocalityId = Guid.Empty then
            getAll ctx filters.Order
        else
            let search = filters.Filter.ToUpper()
            let collection = ctx.Query<Student>("Student")

            let q =
                if filters.LocalityId = Guid.Empty then
                    query {
                    ...
                    query {
                        for student in collection do
                            where (student.LocalityId = filters.LocalityId && student.Name.Contains(search))
                            select student
                            take 50
                    }
